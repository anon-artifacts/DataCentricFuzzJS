...
def load_and_prepare_data(positive_csv: str, negative_csv: str) -> pd.DataFrame:
    ...
    if dropped_cols:
        print(f"⚠️ Dropped columns that could not be converted to a numeric type (booleans/integers are preserved): {', '.join(dropped_cols)}")
...
def select_features_automatically(df: pd.DataFrame) -> list:
    ...
    x_range = range(rfecv.min_features_to_select, rfecv.min_features_to_select + len(scores))
    plt.plot(x_range, scores, marker='o')
    plt.axvline(x=optimal_n_features, color='r', linestyle='--', label=f'Optimal = {optimal_n_features}')
...
def build_model(df: pd.DataFrame, selected_features: list) -> ImbPipeline:
    ...
    pipeline = ImbPipeline([
        ...
        estimator=XGBClassifier(n_estimators=100, tree_method='hist', eval_metric='aucpr', random_state=42,scale_pos_weight=(y==0).sum()/max(1,(y==1).sum()))
        , method='sigmoid',
        cv=3))
    ])
    pipeline.fit(X, y)
...
def evaluate_model_with_cv(df: pd.DataFrame, selected_features: list):
    ...
    pipeline = ImbPipeline([
        ...
        #('classifier', RandomForestClassifier(n_estimators=200, random_state=42, n_jobs=-1))
        #('smote',  SMOTEENN(random_state=42)),
        ('classifier', XGBClassifier(n_estimators=200, use_label_encoder=False, eval_metric='aucpr', random_state=42,scale_pos_weight=(y == 0).sum() / (y == 1).sum()))
    ])
    ...
    report = classification_report(y, y_pred, target_names=['No Crash (0)', 'Crash (1)'], output_dict=True)
    crash_recall = report['Crash (1)']['recall']
    crash_precision = report['Crash (1)']['precision']

    print("Key Metrics Summary:")
    print("--------------------")
    print(f"Recall (Sensitivity) for 'Crash': {crash_recall:.2%} (How many actual crashes were found?)")
    print(f"Precision for 'Crash':          {crash_precision:.2%} (When predicting a crash, how often was it correct?)")
    print(f"False Alarm Rate (FPR):         {false_alarm_rate:.2%} (What percentage of non-crashes were flagged as a crash?)\n")
...
# Train once
    ...
    df_importance['importance_diff'] = df_importance['train_importance'] - df_importance['val_importance']
...
def predict_crash_from_dict(new_data_dict, model_path='final_crash_detection_model.pkl', feature_columns=None):
    """
    ...
    if feature_columns is not None:
        ...
        data = pd.DataFrame([{col: new_data_dict.get(col, 0) for col in feature_columns}])
    else:
...
def load_and_merge_static_dynamic(
    pos_static_csv: str, neg_static_csv: str,
    pos_dynamic_csv: str, neg_dynamic_csv: str
) -> pd.DataFrame:
    """
    ...
    df_pos = pd.concat([df_pos_static.drop(columns=["target"], errors='ignore'),
                        df_pos_dyn.drop(columns=['target', 'exit_code', 'execution_time_ms', 'peak_memory_mb', 'crash_signal', 'error','log_kb','store_ic_count','bucket_misc_count','map_create_count','elem_kind_holey','total_ic_count','load_ic_count'], errors='ignore')], axis=1)
    df_neg = pd.concat([df_neg_static.drop(columns=["target"], errors='ignore'),
                        df_neg_dyn.drop(columns=['target', 'exit_code', 'execution_time_ms', 'peak_memory_mb', 'crash_signal', 'error','log_kb','store_ic_count','bucket_misc_count','map_create_count','elem_kind_holey','total_ic_count','load_ic_count'], errors='ignore')], axis=1)
...
if __name__ == "__main__":
    ...
    master_df = load_and_merge_static_dynamic(
        POS_STATIC, NEG_STATIC,
        POS_DYNAMIC, NEG_DYNAMIC
    )
    #print(master_df.summary())
    
    #if master_df is not None:
    #    all_features = [col for col in master_df.columns if col != "target"]
    #    build_model(master_df, all_features)