...
def find_perfect_features(X, y):
    ...
    for col in X.columns:
        ...
        if vals_pos.nunique() == 1 and vals_neg.nunique() == 1 and vals_pos.iloc[0] != vals_neg.iloc[0]:
...
def run_model_cv(pos_folds, neg_df, feature_set_name, feature_cols, random_seed=42):
    ...
    for fold in range(2, 5):
        ...
        train_pos = pd.concat([pos_folds[i - 1] for i in range(1, fold)]).reset_index(drop=True)
        ...
        common_features = [f for f in feature_cols if f in train.columns and f in test.columns]
        ...
        for col in X_train.columns:
            ...
            #print(f"{col}: train_unique={train_uniques}, test_unique={test_uniques}")

        #sorted_culprits = sorted(culprit_scores.items(), key=lambda x: -x[1])
        #for feat, score in sorted_culprits[:20]:
        #    print(f"{feat}: deviation={score:.4f}")
        #flagged_features = check_perfect_target_correlation(X_train, y_train)
        #print(f"Fold {fold} - Flagged features: {flagged_features}")
        ...
        if feature_set_name == "random":
            ...
            permutation_importance_scores = permutation_importance(model, X_test, y_test)
            #print(f"Permutation importance for {feature_set_name} on fold {fold}:")
            #print(permutation_importance_scores[:20])  # top 20 features
...
if __name__ == "__main__":
    ...
    if len(dynamic_features) == 0:
        ...
        dynamic_features = [f for f in dynamic_features if f not in ['filename', 'target', 'issue_report_date', 'fold']]
    ...
    print("Running experiments for 5 methods with fixed feature fraction=1 using joblib parallelization...")
    ...
    # Organize results
    all_results = {m: {'precision': [], 'recall': [], 'false_alarm': [], 'n_features': len(f)} for m,f in feature_sets.items()}
