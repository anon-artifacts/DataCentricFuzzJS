...
static_features_selected, dynamic_features_selected, feature_ranges, _ = load_features_and_ranges()
...
def extract_score_shap_via_cache(filepath, fid):
    ...
    if row:
        ...
        return prob, shap
    import requests
    static_features_str = extract_features_via_cli(
        filepath, flags="", static="1")
    static_features = parse_feature_dict(static_features_str)
    filtered_static = {k: static_features.get(
        k, 0) for k in static_features_selected}
    ...
    dynamic_features_str = extract_features_via_cli(
        filepath, flags=flags, static="0")
    dynamic_features = parse_feature_dict(dynamic_features_str)
    filtered_dynamic = {k: dynamic_features.get(
        k, 0) for k in dynamic_features_selected}
    ...
    relevant_features = {
        k: all_features[k] for k in selected_feature_names & shap_feature_names if k in all_features}
...
def run_js_fast(parent_filepath, mutated_filepath, selected_static_features, parent_id):
    ...
    try:
        ...
        prob, features_dict = extract_score_shap_via_cache(
        ...
        filtered_features_mutated = {
            k: v for k, v in features_dict_mutated.items() if k in selected_static_features}
...
def run_js_slow(parent_filepath, mutated_filepath, selected_dynamic_features, static_all_features, selected_all_features, flags, parent_id, mutated_id):
    ...
    try:
        ...
        mutated_score = predict_from_file(
            mutated_filepath, static_all_features | features_dict_mutated, selected_all_features)
        if mutated_score < parent_score and mutated_score < 0.5:
            return {}, {}, {}, False, "unequal score", 0.0
        filtered_features_mutated = {
            k: v for k, v in features_dict_mutated.items() if k in selected_dynamic_features}
        ...
        for key in features_dict:
            ...
            if key in feature_ranges:
                ...
                if mutated_num < lower_bound or mutated_num > upper_bound:
                    return {}, {}, {}, False, f"out of range for {key}: {mutated_num} not in [{lower_bound},{upper_bound}]", 0.0
            else:
                # fallback to strict equality if no range available
                if parent_val != mutated_val:
                    return {}, {}, {}, False, f"no range, unequal {key}", 0.0

        return features_dict, features_dict_mutated, filtered_features_mutated, True, "EQUAL===============", mutated_score
    except subprocess.TimeoutExpired:
...
def insert_into_cache(path, id=None):
    ...
    for js_file in js_files:
        ...
        shap_res = requests.post(
            "http://localhost:5000/shap", json=all_features)
        shap_features = shap_res.json().get("shap_values", [])
        # Only keep features indicated by SHAP and present in selected features
        shap_feature_names = set(shap_features)
        selected_feature_names = set(
...
def predict(parent_code: str,
    ...
    try:
        ...
        if retained:
            ...
            if retained:
                all_features = features_dict_mutated | features_dict_mutated_dynamic
...
def main():
    ...
    predict_parser.add_argument(
        "--parent_file", help="Path to JS Parent PoC file")
    predict_parser.add_argument(
        "--mutated_file", help="Path to JS Mutated PoC file")
    ...
    predict_parser.add_argument(
        "--flags", type=str, help="Optional flags as a string")
    ...
    if args.command == "predict":
        if args.parent_file:
            features_dict_parent, features_dict_mutated, filtered_features_mutated, retained = run_js_fast(
                args.parent_file, args.mutated_file, static_features_selected, args.parent_id)
            if retained:
                if args.flags is None:
                    args.flags = ""
                features_dict_parent_dynamic, features_dict_mutated_dynamic, filtered_features_mutated_dynamic, retained, reason = run_js_slow(
                    args.parent_file, args.mutated_file, dynamic_features_selected, features_dict_mutated, static_features_selected+dynamic_features_selected, args.flags, args.parent_id, args.mutated_id)
            if retained:
        ...
        if args.parent_code:
            ...
            try:
                ...
                if retained:
                    if args.flags is None:
                        args.flags = ""
                    features_dict_parent_dynamic, features_dict_mutated_dynamic, filtered_features_mutated_dynamic, retained, reason, mutated_score = run_js_slow(
                        parent_tmp.name,
                        mutant_tmp.name,
                        dynamic_features_selected,
                        features_dict_mutated,
