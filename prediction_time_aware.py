...
def plot_mean_rank_top_features(
    pos_folds,
    neg_df,
    static_features,
    dynamic_features,
    ...
    for run_idx in range(runs):
        rs = run_idx
        for k in range(2, 5):
            train_pos = pd.concat([pos_folds[i - 1] for i in range(1, k)]).reset_index(
                drop=True
            )
            ...
            train, _ = train_test_split(
                train, train_size=0.9, random_state=rs, stratify=train["target"]
            )
            test, _ = train_test_split(
                test, train_size=0.9, random_state=rs + 1, stratify=test["target"]
            )
            ...
            common_cols = [
                ...
                if c not in ("target", "filename", "issue_report_date", "fold")
            ]
            ...
            importance_df = (
                pd.DataFrame(
                    {"feature": common_cols, "importance": model.feature_importances_}
                )
            ...
            for rank, feat in enumerate(top_feats, start=1):
                record = {
                    ...
                    "feature_type": (
                        ...
                        else "dynamic" if feat in dynamic_features else "unknown"
                    ),
                }
    ...
    mean_rank_df = (
        rank_df.groupby(["feature", "feature_type"])["rank"].mean().reset_index()
    )
    ...
    mean_rank_csv = os.path.join(
        plot_folder, f"mean_rank_top_{int(top_fraction*100)}_perc_features.csv"
    )
    ...
    last_fold_df = (
        last_fold_df.groupby(["feature", "feature_type"])["rank"].mean().reset_index()
    )
    ...
    plt.title(
        f"Mean Rank of Top {int(top_fraction*100)}% Features Across Folds and Runs"
    )
...
def run_time_aware_cv(pos_folds, neg_df, selected_features=None, random_state=None):
    ...
    for i in range(1, 4):
        ...
        train_pos = (
            pos_folds[i - 1].sample(frac=0.9, random_state=rs).reset_index(drop=True)
        )
        test_pos = pos_folds[i].sample(frac=0.9, random_state=rs).reset_index(drop=True)

        train_neg = (
            neg_df[neg_df["fold"] < i + 1]
            .sample(frac=0.9, random_state=rs)
            .reset_index(drop=True)
        )
        test_neg = (
            neg_df[neg_df["fold"] == i + 1]
        ...
        common_cols = [
            col
            for col in common_cols
            if col not in ("target", "filename", "issue_report_date", "fold")
        ]
...
def run_time_aware_cv_per_fold_feature_ranking(
    pos_folds, neg_df, top_fractions, random_state=None
):
    results = []
    ...
    for k in range(2, 5):  # folds 2, 3, 4 as test folds
        ...
        feature_importance_df = pd.DataFrame(
            {"feature": common_cols, "importance": base_model.feature_importances_}
        ).sort_values(by="importance", ascending=False)
...
def plot_metrics_with_variance(results_all_runs):
    ...
    all_df = pd.concat(
        results_all_runs, keys=range(len(results_all_runs)), names=["run", "index"]
    ...
    plt.title(
        "Time-aware Cross-Validation Metrics with Variance over 10 Runs",
        fontsize=16,
        pad=20,
    )
...
def plot_combined_metrics_single_fraction(results_all_runs, top_fraction):
    ...
    fa_upper_bound = (
        max(fa_mean + error_scale * fa_std) * 3.0
    )  # doubled to lower the curve
    ax2.errorbar(
        folds,
        fa_mean,
        yerr=error_scale * fa_std,
        fmt="-^",
        capsize=5,
        label="False Alarm",
        color="tab:red",
    ...
    filename = os.path.join(
        PLOT_FOLDER, f"metrics_combined_top_fraction_{top_fraction}.png"
    )
...
def plot_individual_metric(results_all_runs, metric_name, top_fraction):
    ...
    plt.title(
        f"{metric_name.capitalize()} over Time-aware CV (Top Fraction={top_fraction})",
        fontsize=16,
    )
    ...
    filename = os.path.join(
        PLOT_FOLDER, f"{metric_name}_top_fraction_{top_fraction}.png"
    )
...
def plot_combined_metrics(results_all_runs, top_fractions):
    ...
    y_limits = {
        ...
        "false_alarm": (
            0,
            max(0.05, all_df["false_alarm"].max() * 1.2),
        ),  # tighter for low values
    }
    ...
    for metric in metrics:
        ...
        plt.title(
            f"{metric.capitalize()} vs Fold for Different Feature Fractions",
            fontsize=14,
        )
...
def find_diminishing_point(agg_df, improvement_threshold=0.005):
    ...
    agg_df["gain"] = agg_df[agg_df.columns[1]].diff() / agg_df[agg_df.columns[1]].shift(
        1
    )
...
def plot_diminishing_returns(
    results_all_runs, metric="precision", improvement_threshold=0.005
):
    ...
    if frac_point:
        ...
        plt.axvline(
            ...
            color="red",
            linestyle="--",
            label=f"Diminishing point = {frac_point:.2f}",
        )
...
def find_statistically_minimal_fraction(
    results_all_runs,
    metric="precision",
    alpha=0.05,
    test="wilcoxon",
...
def wilcoxon_test_with_effect_size(df, ref_frac=0.1, metrics=None):
    ...
    print(
        "\n=� Wilcoxon Rank Test: Comparing {:.1f} fraction vs other fractions".format(
            ref_frac
        )
    )
    for metric in metrics:
...
if __name__ == "__main__":
    ...
    static_features_set, dynamic_features_set = (
        get_static_and_dynamic_features_from_neg_files(
    ...
    for frac in top_fractions:
        ...
        for run_idx in range(10):
            results_df = run_time_aware_cv_per_fold_feature_ranking(
                pos_folds, neg_df, top_fractions, random_state=run_idx
            )
